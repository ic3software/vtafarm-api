...
- **Share** — a `full_stack` owner mints a connection bundle for their stack and
  hands it to somebody out of band — §4.
...
half-built — the API has no code path for them and the bundle format does not
...
| A share code, minted and rotated by the stack's owner | a directory of stacks accepting connections |
...
> **The bundle is evidence, not configuration.**

The three values a consumer session is built from are read from the **provider's
row in our own database**. The pasted bundle is used only to (a) name which row,
(b) prove the sharer authorised it, and (c) show the recipient what they are
joining before they commit.
...
- **No stale-value class of bug.** A rebuilt stack is a *different row* with a
  different share code, so an old bundle fails to resolve instead of
  provisioning against a daemon that no longer exists.

A design that instead took the URLs from the bundle would need every mitigation
in §9.4 to be correct forever. This one needs none of them.

---

## 4. Share: the connection bundle
...
| rotate | mint a new one — outstanding bundles stop working, live connections keep running |
...
path, which sends no bundle at all. That also means nobody can accidentally
paste their way onto it through the Customize form — the two paths stay
distinct.

### 4.2 What the bundle contains

```json
{
  "v": 1,
  "kind": "vtafarm.stack-connection",
  "farm": "firstperson.dev",
  "stack": "alice",
  "code": "k7m2-9xqp-4b8w-3nrt",
  "mediator_did": "did:webvh:…:mediator-alice.firstperson.dev",
  "did_hosting_server_url": "https://dids-alice.firstperson.dev",
  "did_hosting_did": "did:webvh:…:dids-alice.firstperson.dev"
}
```

Two fields do the work; the rest is for humans and for §5.2's confirmation card.

| Field | Role |
| --- | --- |
| `stack` | the provider's `vta_name` — its public identifier everywhere else in this API (`CLAUDE.md`, "a session is addressed by its name"), so the lookup key is the one users already see |
| `code` | the grant (§4.1) |
| `farm` | lets the paste side say "this bundle isn't from this farm" instead of "stack not found" |
| `v` / `kind` | so pasting a DID, a URL or a JWT fails with a sentence rather than a 500 five steps later |
| `mediator_did`, `did_hosting_server_url`, `did_hosting_did` | **displayed and compared, never used** (§3). Compared so a bundle showing values the stack no longer has is refused rather than silently connecting to different ones than the recipient was shown |

Every value already exists on the provider's row and is already returned by
`GET /setup/:id` (`collected.mediator_did`, `collected.did_hosting_did`,
`urls.dids`). **The bundle invents no data** — only the code is new, and it is
minted by an explicit act.
...
bundle that would fail on arrival.
...
```jsonc
{
  ...
  "vta_name": "myvta",
  "vta_image": "ghcr.io/…",
  "connection": { /* the §4.2 bundle, verbatim */ }   // optional
}
```
...
// ref != nil  → the stack the bundle names
...
**Tier 1 — is this bundle usable at all.** One reason for every outcome:
...
same fact from the holder's point of view — *this bundle does not currently open
anything* — and the only honest next step for all five is the same one: ask the
owner for a current bundle.
...
| bundle's three display values equal the row's | 422 `stack_changed` |
...
The bundle is JSON; a frontend can parse it and render "connecting to **alice**,
mediator `did:webvh:…`" the moment it is pasted. But every one of those values
came **out of the pasted text**. A confirmation card built that way is
self-asserted — it shows a green tick for a bundle whose code is pure garbage,
and the user discovers the truth only after naming their agent, picking an image
and pressing Create.
...
- **It is an oracle, and that is fine.** 75 bits of entropy behind an
  ...
  reason means the oracle answers exactly one question: *does the bundle I was
...
- **It is not authoritative.** `POST /setup` re-runs §5.1 in full. A stack can
  ...
  validate passed.
- **This reverses an earlier decision.** A "test this bundle" *button* was
...
### 5.3 A bundle is one stack's worth of values

Taking the mediator from stack A and the DID host from stack B is refused
implicitly: the bundle names one stack and every value comes from that row.
...
| `GET /setup/:id` (provider, `full_stack`) | `connection` bundle (§4.3) + `connections[]` — the dependents' names and statuses |
...
code, so an old bundle fails at §5.1 step 3. If a stack somehow keeps its code
while its values change, step 5 catches it. Belt and braces, because the failure
mode is an agent provisioned against a daemon that no longer exists.
...
first change that makes URLs bundle-sourced reopens all of it:
...
rests on it — including §4.2's choice of `stack` as the bundle's lookup key. Fix
the message.
...
as the caller brings a bundle.** The platform stack is a default, not a
...
would have to replace §5.1's DB lookup with something else entirely — the bundle
would become configuration again, losing §3. Treat it as a different feature
...
giving it a code and making the default path just a preselected bundle — would
